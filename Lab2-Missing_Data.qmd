...
The dataset has 4106 observations and 5 variables. The majority of data is complete (96.7%). The `lpr` variable has the most missingness (7.8%).

# 2. Missing Data Patterns

-   Use the `naniar fucntion plot_pattern()` function to do a missing data pattern analysis. Adjust the code from lecture to do a similar analysis with variables `iqv`, `ses`, `lpr` and `lpo`.
...
-   In a sentence or two, tell me how many cases showed the most popular pattern and describe to me what that pattern is.
    ...
    The most popular pattern is the first pattern. This pattern describes the number of entries (3464) that have no missingness on any of the variables.

Which variable had the largest number of missing observations?

`lpr` has the largest number of missing observations (320) (7.8%).
...
-   Using what we learned in class, can you determine if the missingness observed `d` is MAR or MCAR? How? Show me.
...
-   What is your conclusion?
    ...
    There is evidence that the data are MAR. This was supported by conducting several logistic regressions where each predictor variable was regressed onto the missing dummy variable.
...
-   In our data statement, enter in `select(my_md_missing, iqv:lpo)`
...
-   Run `mice` to fit a model with multiple imputated data sets (make sure to not include extraneous variables
...
-   Try out the cool `modelsummary` fucntion from `modelsummary(https://vincentarelbundock.github.io/modelsummary/articles/modelsummary.html)` to compare the listwise deletion, mean, and regression imputation objects above. Do you see any differences in the estimates, *SEs*, and $R^2$, *p*-values?
